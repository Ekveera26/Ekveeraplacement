...
async function runSearchCandidate() {
  ...
  try {
    ...
    if (lastSearchCandResults.length) {
      el.insertAdjacentHTML('afterbegin', '<p style="padding:6px 4px 12px;color:#c9d6e8;font-size:13.5px;">✅ ' + lastSearchCandResults.length + ' नतीजे मिले — नीचे "⬇ Excel Download" बटन से Download कर सकते हैं।</p>');
    }
  } catch (err) {
    el.innerHTML = '<p style="padding:18px;color:#C0392B;">कुछ गड़बड़ हो गई: ' + err.message + '</p>';
  }
}
...
async function runSearchCompany() {
  ...
  try {
    ...
    if (lastSearchCompResults.length) {
      el.insertAdjacentHTML('afterbegin', '<p style="padding:6px 4px 12px;color:#c9d6e8;font-size:13.5px;">✅ ' + lastSearchCompResults.length + ' नतीजे मिले — नीचे "⬇ Excel Download" बटन से Download कर सकते हैं।</p>');
    }
  } catch (err) {
    el.innerHTML = '<p style="padding:18px;color:#C0392B;">कुछ गड़बड़ हो गई: ' + err.message + '</p>';
  }
...
function downloadSearchResults(which) {
  ...
  if (!rows || !rows.length) {
    alert('पहले खोज लीजिए (कम-से-कम 1 नतीजा आना चाहिए), उसके बाद Download करें।');
    return;
  }
  try {
    vcDownloadXlsx(rows, 'Results', which === 'cand' ? 'EkVeera_Candidates' : 'EkVeera_Companies');
    alert('✅ Excel Download हो गया है! अपने Phone/Computer के "Downloads" Folder में देखें। (हर बार दोबारा Download कर सकते हैं)');
  } catch (err) {
    alert('Excel बनाने में दिक्कत आई: ' + err.message + '\n\nकृपया इस Message का Screenshot EkVeera को भेजें।');
  }
}
